            ...
            <main class="flex-1 space-y-12">
                <section class="bg-white rounded-[2.5rem] p-12 shadow-sm border border-gray-100">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                        ...
                        <button onclick="toggleAddressForm()" class="bg-[#3d2b1f] text-white px-8 py-4 rounded-2xl font-bold hover:bg-[#c6a664] transition-all shadow-lg flex items-center gap-3">
                    ...
                    <div id="addressFormSection" class="hidden mb-12 p-10 bg-[#fdfbf7] rounded-[2.5rem] border border-gray-100">
                        <form id="addressForm" action="{{ route('dashboard.addresses.save') }}" method="POST" class="grid md:grid-cols-2 gap-8">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="title" required placeholder="Home" class="w-full bg-white border-none rounded-2xl py-4 px-6 text-sm font-bold text-[#3d2b1f] focus:ring-2 focus:ring-[#c6a664]">
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-4">Landmark (Optional)</label>
                                <input type="text" name="landmark" placeholder="Near Central Park" class="w-full bg-white border-none rounded-2xl py-4 px-6 text-sm font-bold text-[#3d2b1f] focus:ring-2 focus:ring-[#c6a664]">
                            </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-8">
                        @forelse($addresses as $address)
                            <div class="group p-8 bg-[#fdfbf7] rounded-[2.5rem] border {{ $address->is_primary ? 'border-[#c6a664]/30 shadow-xl shadow-[#c6a664]/5' : 'border-gray-50' }} hover:bg-white hover:shadow-2xl transition-all duration-500 relative overflow-hidden">
                                ...
                                <div class="flex items-start gap-6">
                                    <div class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-[#c6a664] shadow-sm flex-shrink-0">
                                    ...
                                    <div class="flex-1">
                                        <h5 class="text-xl font-bold text-[#3d2b1f] mb-3">{{ $address->title }}</h5>
                                        <p class="text-sm text-gray-500 leading-relaxed mb-4">{{ $address->full_address }}</p>
                                ...
                                <div class="mt-8 pt-6 border-t border-white/50 flex gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button class="text-xs font-bold text-[#3d2b1f] hover:text-[#c6a664] transition-all">Edit Address</button>
                                    <button onclick="deleteAddress({{ $address->id }})" class="text-xs font-bold text-red-400 hover:text-red-600 transition-all">Remove</button>
                                </div>
                            </div>
                        @empty
...
<script>
    function toggleAddressForm() {
        const form = document.getElementById('addressFormSection');
